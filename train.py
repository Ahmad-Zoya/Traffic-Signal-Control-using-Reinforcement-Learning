...
class Model(nn.Module):
    def __init__(self, lr, fc1_dims, fc2_dims, n_actions, input_dims=2):
...
class Agent:
    ...
    def store_transition(self, state, state_, action,reward, done,junction):
        ...
        if len(state_) < 4:
            # Pad with zeros to make the shape (4,)
            padding = np.zeros(4 - len(state_), dtype=np.float32)
            state_ = np.concatenate((state_, padding))
    ...
    def choose_action(self, observation):
        ...
        state = state.unsqueeze(0)  # Add a batch dimension (batch_size=1)
        if state.shape[1] < 4:
            # Pad with zeros to make the shape (batch_size=1, input_dims=4)
            padding = torch.zeros(1, 4 - state.shape[1], dtype=torch.float).to(self.Q_eval.device)
...
def run(train=True,model_name="model",epochs=50,steps=500,ard=False):
    ...
    select_lane = {}  # Dictionary to hold lane states for each junction

    for junction in all_junctions:
        lanes = traci.trafficlight.getControlledLanes(junction)
        num_lanes = len(lanes)
        select_lane[junction] = {
            0: "r" * num_lanes + "G" + "r" * (num_lanes - 1),
            1: "r" * num_lanes + "G" * (num_lanes - 1) + "y",
        }
        ...
        while step <= steps:
            ...
            for junction_number, junction in enumerate(all_junctions):
                ...
                if traffic_lights_time[junction] == 0:
                    ...
                    # vehicles_per_lane = list(get_vehicle_numbers(controled_lanes).values())
                    vehicles_per_lane = dict(enumerate(get_vehicle_numbers(controled_lanes)))
                    #  vehicles_per_lane = get_vehicle_numbers(all_lanes)

                    #storing previous state and current state
                    reward = -1 *  waiting_time
                    state = np.array(prev_vehicles_per_lane[junction_number], dtype=np.float32)
                    ...
                    observation = torch.tensor(list(vehicles_per_lane.values()), dtype=torch.float)
                    observation = observation.unsqueeze(0)  # Add a batch dimension (batch_size=1)
                    if observation.shape[1] < 4:
                        # Pad with zeros to make the shape (batch_size=1, input_dims=4)
                        padding = torch.zeros(1, 4 - observation.shape[1], dtype=torch.float)
                        observation = torch.cat((observation, padding), dim=1)
                    lane = brain.choose_action(observation)

                    state = torch.tensor(prev_vehicles_per_lane[junction_number], dtype=torch.float)
                    state = state.unsqueeze(0)  # Add a batch dimension (batch_size=1)
                    if state.shape[1] < 4:
                        # Pad with zeros to make the shape (batch_size=1, input_dims=4)
                        padding = torch.zeros(1, 4 - state.shape[1], dtype=torch.float)
                        state = torch.cat((state, padding), dim=1)

                    prev_vehicles_per_lane[junction_number] = state

                    prev_action[junction_number] = lane
                    phaseDuration(junction, 6, select_lane[junction][lane][0])  # Update this line
                    phaseDuration(junction, min_duration + 10, select_lane[junction][lane][1])
